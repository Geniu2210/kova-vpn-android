...
class MainActivity : HelperBaseActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    private fun setupPager() {
        ...
        pagerAdapter = GroupPagerAdapter(this, groups)
        binding.viewPager.adapter = pagerAdapter
        TabLayoutMediator(binding.tabLayout, binding.viewPager) { tab, position ->
            tab.text = groups.getOrNull(position)?.remarks ?: ""
        }.attach()
        if (groups.size <= 1) {
    ...
    private fun setupViewModel() {
        mainViewModel.updateListAction.observe(this) { index ->
            // Defer update until after any ongoing fragment transactions
            binding.viewPager.post {
                val groups = mainViewModel.getSubscriptions(this)
                pagerAdapter?.update(groups)
                if (groups.size <= 1) {
                    binding.tabLayout.visibility = View.GONE
                } else {
    ...
    private fun importConfigLocal(): Boolean {
        try { showFileChooser() } catch (e: Exception) {
